...
# define header box actors

headerbox = Rect(0, 0, WIDTH, header)

# initialise other game variables
...
function draw(g::Game)
    ...
    if gameover == false
        ...
        replay = TextActor("Click to Play Again", "gargi";
            font_size = 36, color = Int[0, 0, 0, 255]
        )
        replay.pos = (135, 390)
        draw(replay)
    end
    txt = TextActor(display, "gargi";
...
function update(g::Game)
    if gameover == false
        ...
        collide_head_apple()
        grow()
        popat!(snake_body,1) # remove oldest position of snake
